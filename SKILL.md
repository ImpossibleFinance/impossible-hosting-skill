...
3. **Stream long commands, don't poll-and-check.** For anything expected to run >30s (builds, installers, Chromium downloads, console installs), attach a `Monitor` with a diff-based tail that emits NEW lines as they appear AND a heartbeat when no new lines for 60s. `tail -1` + sleep 15 is a bug — you see one line per minute and miss errors.
...
> **Known upstream issue (as of 2026-04-22):** OpenClaw's Telegram webhook handler calls the LLM synchronously before returning 200 to Telegram, which blows Telegram's webhook read timeout on slow responses. Symptom: `last_error_message: "Read timeout expired"` in `getWebhookInfo`, pending updates pile up, replies lag 1-5 min. This is in OpenClaw's code — `webhookCallback` is called with `onTimeout: "return"` but `timeoutMilliseconds: 10_000`, which is longer than Telegram's actual read tolerance for LLM-bound handlers. A Cloudflare Worker webhook relay (see "Production reliability" below) fully works around it. Upstream fix proposed in [openclaw/openclaw#70146](https://github.com/openclaw/openclaw/pull/70146) (lower the constant to 5_000).
